...
/**
 * Notifica entregador por e-mail e push quando o cadastro sai de "pendente"
 * (ou "bloqueado") para "aprovado"/"ativo" no painel master, ou quando é
 * recusado (bloqueado por recusa cadastral).
 *
 * Espelha a estrutura de `lojista_status_notificacao.js`, adaptada ao campo
 * `entregador_status` e ao perfil de entregador.
 */
        ...
        <table role="presentation" width="100%" style="max-width:560px;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 12px 40px rgba(106,27,154,0.14);">
          <tr>
            <td style="background:linear-gradient(135deg,#6A1B9A 0%,#8E24AA 55%,#7B1FA2 100%);padding:34px 28px;text-align:center;">
              ...
              <p style="margin:14px 0 0;color:rgba(255,255,255,0.95);font-size:15px;line-height:1.45;">Seu cadastro de entregador foi validado.</p>
            </td>
          </tr>
          <tr>
            <td style="padding:34px 32px 26px;">
              ...
              <p style="margin:0 0 18px;font-size:15px;line-height:1.65;color:#444;">
                Sua conta de <strong>entregador</strong> foi <strong style="color:#0f766e;">aprovada</strong> no DiPertin.
              </p>
                ...
                <tr>
                  <td style="padding:18px 18px;">
                    <p style="margin:0;font-size:14px;line-height:1.55;color:#14532d;">
                      Você já pode abrir o app, ficar online e começar a receber ofertas de corrida.
                    </p>
                  </td>
                </tr>
              </table>
              <p style="margin:0;font-size:14px;line-height:1.65;color:#555;">
                Lembre-se de manter localização, notificações e bateria permitidas para receber chamadas em tempo real.
              </p>
            </td>
          </tr>
          <tr>
...
<head>
  ...
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <meta http-equiv="x-ua-compatible" content="ie=edge">
  <title>Atualização do cadastro de entregador</title>
</head>
<body style="margin:0;background:#f5f4f8;font-family:'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#1a1a2e;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f5f4f8;padding:36px 16px;">
    <tr>
          ...
          <tr>
            <td style="padding:34px 32px 26px;">
              ...
              <p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:#444;">
                Seu cadastro de entregador foi analisado e, no momento, não pôde ser aprovado.
              </p>
...
function textoPlanoAprovado(nome) {
    return [
        `Olá, ${nome}!`,
        "",
        "Sua conta de entregador foi aprovada no DiPertin.",
        "Você já pode abrir o app, ficar online e começar a receber ofertas de corrida.",
        "",
        "Mantenha localização, notificações e bateria permitidas para receber chamadas.",
        "",
...
function textoPlanoRecusado(nome, motivo) {
    return [
        `Olá, ${nome}!`,
        "",
        "Seu cadastro de entregador foi analisado e não pôde ser aprovado neste momento.",
...
async function enviarEmailStatus(destinoEmail, nome, evento, motivoRecusa, uid) {
    ...
    try {
        ...
        console.warn("[entregador-status] SMTP não configurado. E-mail não enviado:", e.message);
        return false;
    }
    ...
    const subjectBase = aprovado
        ? "DiPertin — sua conta de entregador foi aprovada"
        : "DiPertin — atualização do seu cadastro de entregador";
    const subject = aprovado ? subjectBase : _subjectComMarcador(subjectBase);

    const host = _hostDoEmail(from);
    const random = Math.random().toString(36).slice(2, 12);
    const messageId = `<entregador-status-${evento}-${uid || "x"}-${Date.now()}-${random}@${host}>`;

    console.log(
        `[entregador-status] SMTP enviando → evento=${evento} para=${destinoEmail} uid=${uid || "?"} msgId=${messageId}`,
    );
    ...
    try {
        ...
        console.log(
            `[entregador-status] SMTP OK → aceitos=${accepted.length} rejeitados=${rejected.length} resposta=${responseServidor}`,
        );

        if (rejected.length > 0 && accepted.length === 0) {
            console.error(
                `[entregador-status] SMTP TODOS OS DESTINOS REJEITADOS → rejeitados=${JSON.stringify(rejected)}`,
            );
            return false;
        }
        return true;
        ...
        console.error(
            `[entregador-status] SMTP FALHOU → evento=${evento} uid=${uid || "?"} err=${e && e.message ? e.message : e}`,
        );
        throw e;
    }
}
...
async function enviarPushStatus(db, uid, evento, motivoRecusa) {
    const { token, ok } = await notificationDispatcher.obterTokenValidado(db, uid, "entregador");
    if (!ok || !token) return false;

    const aprovado = evento === "aprovada";
    const titulo = aprovado ? "Conta aprovada" : "Cadastro não aprovado";
    const bodyBase = aprovado
        ? "Seu cadastro de entregador foi aprovado. Abra o app para ficar online."
        : "Seu cadastro de entregador não foi aprovado. Verifique o motivo e atualize seus dados.";
    ...
    const data = notificationDispatcher.dataSoStrings({
        type: aprovado ? "ENTREGADOR_CADASTRO_APROVADO" : "ENTREGADOR_CADASTRO_RECUSADO",
        tipoNotificacao: aprovado ? "entregador_cadastro_aprovado" : "entregador_cadastro_recusado",
        segmento: "entregador",
    ...
    await admin.messaging().send({
        ...
        android: {
            ...
            collapseKey: aprovado
                ? `entregador_cadastro_aprovado_${uid}`
                : `entregador_cadastro_recusado_${uid}`,
            notification: {
...
exports.onEntregadorStatusCadastroAtualizado = functions.firestore
    .document("users/{uid}")
    .onUpdate(async (change, context) => {
        ...
        const statusAntes = normalizarStatus(antes.entregador_status);
        ...
        // --- RECUSA: dispara quando transitou para bloqueado OU quando já
        // estava bloqueado mas o motivo mudou (painel re-classificou recusa).
        if (!evento && STATUS_BLOQUEADO.has(statusDepois)) {
            ...
            const transitouParaBloqueado =
                !STATUS_BLOQUEADO.has(statusAntes) && statusAntes !== statusDepois;
            const reclassificouMotivo =
                STATUS_BLOQUEADO.has(statusAntes) &&
                recusaCadastro &&
                (motivoAntes !== motivoDepois || codigoAntes !== codigoDepois);

            if (recusaCadastro && (transitouParaBloqueado || reclassificouMotivo)) {
                evento = "recusada";
            }
        }
        ...
        console.log(
            `[entregador-status] evento=${evento} uid=${uid} email=${email || "(vazio)"} motivoLen=${motivoRecusa.length}`,
        );
        ...
        if (validarEmail(email)) {
            try {
                ...
                console.error(`[entregador-status] Falha e-mail uid=${uid}:`, e.message || e);
            }
        } else {
            console.warn(`[entregador-status] E-mail inválido/ausente uid=${uid}`);
        }
        ...
        try {
            ...
            console.error(`[entregador-status] Falha push uid=${uid}:`, e.message || e);
        }

        try {
            await change.after.ref.set(
                {
                    notif_cadastro_entregador_ultimo_evento: evento,
                    notif_cadastro_entregador_em: admin.firestore.FieldValue.serverTimestamp(),
                    notif_cadastro_entregador_email_ok: emailOk,
                    notif_cadastro_entregador_push_ok: pushOk,
                },
                { merge: true },
            );
        } catch (e) {
            console.warn(`[entregador-status] Falha ao persistir auditoria uid=${uid}:`, e.message || e);
        }
